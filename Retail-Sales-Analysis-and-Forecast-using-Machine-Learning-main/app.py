...
def get_local_sql_df():
    sql_path = data_root() / 'dataset' / 'df_sql.csv'
    if sql_path.exists():
        df = pd.read_csv(sql_path)
        # Map column names to lowercase with underscores
        df.columns = [c.lower().replace(' ', '_') for c in df.columns]
        # Fix specific column names to match database schema
        rename_map = {
...
def get_local_store_df():
    store_path = data_root() / 'dataset' / 'stores_data_set.csv'
...
def parse_condition(condition):
    if condition is None:
        return ''
    expr = condition
    expr = re.sub(r"([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*'([^']*)'", r"\1 == \2", expr)
    expr = re.sub(r"([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([0-9]+)", r"\1 == \2", expr)
    return expr


def filter_local_df(condition, df=None):
    if df is None:
        df = get_local_sql_df()
    if df is None:
        return pd.DataFrame()
...
class sql:
    ...
    def data_migration():
        ...
        try:
            csv_path = data_root() / 'dataset' / 'df_sql.csv'
...
class top_sales:
    ...
    @staticmethod
    def _local_sql_df():
        return get_local_sql_df()

    @staticmethod
    def _filter_local(condition, df=None):
        return filter_local_df(condition, df)
    ...
    def sql(condition):
        try:
            ...
            df = top_sales._filter_local(condition)
            if df.empty:
                return df
    ...
    def day(month, year):
        try:
            ...
            df = top_sales._local_sql_df()
            if df is not None:
                return sorted(df.loc[(df['year'] == year) & (df['month'] == month), 'day'].dropna().astype(int).unique().tolist())
            return []
    ...
    def store(day, month, year):
        try:
            ...
            df = top_sales._local_sql_df()
            if df is not None:
                return sorted(df.loc[(df['year'] == year) & (df['month'] == month) & (df['day'] == day), 'store'].dropna().astype(int).unique().tolist())
            return []
    ...
    def dept(day, month, year, store):
        try:
            ...
            df = top_sales._local_sql_df()
            if df is not None:
                return sorted(df.loc[(df['year'] == year) & (df['month'] == month) & (df['day'] == day) & (df['store'] == store), 'dept'].dropna().astype(int).unique().tolist())
            return []
    ...
    def top_store_dept(day, month, year):
        try:
            ...
            df = top_sales._local_sql_df()
            if df is not None:
                data = sorted(df.loc[(df['year'] == year) & (df['month'] == month) & (df['day'] == day), 'dept'].dropna().astype(int).unique().tolist())
                data.insert(0, 'Overall')
                return data
            return ['Overall']
    ...
    def top_store_sales(condition):
        try:
            ...
            if df.empty:
                return df
            df = df.groupby('store', as_index=False)['weekly_sales'].sum().sort_values('weekly_sales', ascending=False).head(10)
            df['Weekly Sales'] = df['weekly_sales'].apply(lambda x: f"{x:.2f}")
            df['store_x'] = df['store'].apply(lambda x: str(x)+'*')
            df = df[['store', 'Weekly Sales', 'store_x']]
            df.index = range(1, len(df)+1)
            df.index.name = 's.no'
            return df
    ...
    def top_dept_store(day, month, year):
        try:
            ...
            df = top_sales._local_sql_df()
            if df is not None:
                data = sorted(df.loc[(df['year'] == year) & (df['month'] == month) & (df['day'] == day), 'store'].dropna().astype(int).unique().tolist())
                data.insert(0, 'Overall')
                return data
            return ['Overall']
    ...
    def top_dept_sales(condition):
        try:
            ...
            if df.empty:
                return df
            df = df.groupby('dept', as_index=False)['weekly_sales'].sum().sort_values('weekly_sales', ascending=False).head(10)
            df['Weekly Sales'] = df['weekly_sales'].apply(lambda x: f"{x:.2f}")
            df['dept_x'] = df['dept'].apply(lambda x: str(x)+'*')
            df = df[['dept', 'Weekly Sales', 'dept_x']]
            df.index = range(1, len(df)+1)
            df.index.name = 's.no'
            return df
...
class comparison:
    ...
    def sql(condition):
        try:
            ...
            df = comparison._filter_local(condition)
            if not df.empty:
                df.index = range(1, len(df) + 1)
                df.index.name = 's.no'
            return df
    ...
    def previous_week_day(month, year):
        try:
            ...
            df = comparison._local_sql_df()
            if df is not None:
                data = sorted(df.loc[(df['year'] == year) & (df['month'] == month), 'day'].dropna().astype(int).unique().tolist())
                if month == 2 and year == 2010 and len(data) > 0:
                    data.remove(data[0])
                return data
            return []
    ...
    def top_store_sales(condition):
        try:
            ...
            if df.empty:
                return df
            agg_cols = {'weekly_sales': 'sum', 'temperature': 'mean', 'fuel_price': 'mean',
                       'markdown1': 'mean', 'markdown2': 'mean', 'markdown3': 'mean',
                       'markdown4': 'mean', 'markdown5': 'mean', 'cpi': 'mean',
                       'unemployment': 'mean'}
            result = df.groupby(['store', 'type', 'size', 'is_holiday'], as_index=False).agg(agg_cols)
            result = result.sort_values('weekly_sales', ascending=False)
            result['weekly_sales'] = result['weekly_sales'].apply(lambda x: f'{x:.2f}')
...
class features:
    ...
    def sql_sum_avg(condition):
        try:
            ...
            df = features._filter_local(condition)
            if df.empty:
                return df
            agg_cols = {'weekly_sales': 'sum', 'temperature': 'mean', 'fuel_price': 'mean',
                       'markdown1': 'mean', 'markdown2': 'mean', 'markdown3': 'mean',
                       'markdown4': 'mean', 'markdown5': 'mean', 'cpi': 'mean', 'unemployment': 'mean'}
            result = df.groupby(['store', 'type', 'size', 'is_holiday'], as_index=False).agg(agg_cols)
            result = result.sort_values('weekly_sales', ascending=False)
            result.index = range(1, len(result)+1)
            result.index.name = 's.no'
    ...
    def store():
        try:
            ...
            df = features._local_sql_df()
            if df is not None:
                return sorted(df['store'].dropna().astype(int).unique().tolist())
            return []
    ...
    def sql_holiday(condition):
        try:
            ...
            if df.empty:
                return pd.DataFrame()
            agg_df = df.groupby('is_holiday', as_index=False)['weekly_sales'].mean()
            agg_df.columns = ['is_holiday', 'weekly_sales']
            agg_df['weekly_sales'] = agg_df['weekly_sales'].apply(lambda x: f"{x:.2f}")
            agg_df['decode'] = agg_df['is_holiday'].apply(lambda x: 'Yes' if x==1 else 'No')
            agg_df = agg_df.drop(columns=['is_holiday'])
            agg_df.index = range(1, len(agg_df)+1)
            agg_df.index.name = 's.no'
            return agg_df
...
class prediction:
    ...
    def type_size_dict():
        try:
            ...
            store_df = get_local_store_df()
            if store_df is None:
                st.warning('PostgreSQL unavailable and local store metadata is missing.')
                return {}, {}
            type_map = {'A': 1, 'B': 2, 'C': 3}
            store_df['type'] = store_df['type'].astype(str).map(type_map).fillna(store_df['type']).astype(int)
            store_df['store'] = store_df['store'].astype(int)
            store_df['size'] = store_df['size'].astype(int)
            return dict(zip(store_df['store'], store_df['type'])), dict(zip(store_df['store'], store_df['size']))
    ...
    def dept(store):
        try:
            ...
            if df is None:
                return []
            data = sorted(df.loc[df['store'] == store, 'dept'].dropna().astype(int).unique().tolist())
            return data
    ...
    def predict_weekly_sales():
        ...
        if button:
            with st.spinner(text='Processing...'):
                ...
                model_path = data_root() / 'model' / 'model1_markdown.pkl'
                if model_path.exists():
                    ...
                    df = get_local_sql_df()
                    if df is None:
                        st.error('Missing prediction model and local training data. Please provide model/model1_markdown.pkl.')
                        return None
                    from sklearn.linear_model import LinearRegression
                    X = df[['day', 'month', 'year', 'store', 'dept', 'type', 'size', 'is_holiday',
                ...
                if store not in type_dict or store not in size_dict:
                    st.error('Selected store not found in fallback data. Please choose a different store.')
                    return None
