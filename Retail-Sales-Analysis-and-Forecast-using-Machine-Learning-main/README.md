...
To run this project, use the shared requirements file:

```bash
pip install -r requirements.txt
```

The application is configured to use local CSV fallback data, so PostgreSQL is optional. If you want to use the SQL migration feature, create a PostgreSQL database named `retail_forecast` and update the connection credentials in `app.py` if needed.
...
This repository is ready for GitHub-based deployment. The app uses local CSV data by default, so it can run without PostgreSQL. For hosted deployment, use one of these options:

- **Streamlit Cloud**: Connect this GitHub repository and set the app path to `app.py`.
- **Heroku / Railway / Render**: Use the included `Procfile`:
  ```text
  web: streamlit run app.py --server.port $PORT --server.address 0.0.0.0
  ```
- **GitHub Actions**: A workflow is included to validate the app on push.
